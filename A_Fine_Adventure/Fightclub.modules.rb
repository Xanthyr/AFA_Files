...
module Rng # RNG engine via dice emulation
 ...
 rolls.times do
   ...
   @damage = @allRolls + @ability_mod
    end
  end
end

#  Narration of what the dice are actually doing
# module Narratable
#   def rollResult
# end
# Creation tools
...
module Combat
  ...
  def basic_attack(target)
    ...
    if @damage >= target.armor_class
      ...
      puts "\n#{self.name} attacks #{target.name} with their #{@weapon} for [#{@rolls}+#{@ability_mod}]=#{@damage} points of damage!\n"
      target.hurt(@damage)
      puts "\n"
    else
...
class Wizard < Dude
  def fireball(target)
    ...
    diceroll(3, 6)
    puts "#{target.name} is fireballed for [#{@rolls}+#{@ability_mod}]=#{@damage} damage!"
    target.hurt(@damage)
...
class Fighter < Dude
    ...
    diceroll(1, 4)
    puts "#{self.name} finishes with a wide swing of the #{@weapon} for [#{@rolls}+#{@ability_mod}]=#{@damage} points of damage. Dang!"
      target.hurt(@damage)
    end
end
...
class Cleric < Dude
  ...
  def cure_wounds(target, lvl)
    ...
    diceroll(lvl, 8)
    target.heal(@damage)
    puts "#{target.name} is healed for #{@rolls}+#{@ability_mod}=#{@damage} health points!"
  end
end
